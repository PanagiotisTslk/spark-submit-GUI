...
def edit_timestamp(job, choice, action):
    col = database_connection('Jobs')
    ...
    if action == 'add':
        ...
        if choice == 'started':
            # result = col.update_one({"_id": ObjectId(job['_id'])}, {"$unset": {'started_timestamp': 1}})
            result = col.update_one({"_id": ObjectId(job['_id'])}, {"$set": {'status': 'failed', 'priority': 2}})
        elif choice == 'finished':
            result = col.update_one({"_id": ObjectId(job['_id'])}, {"$unset": {'finished_timestamp': 1}})
            result = col.update_one({"_id": ObjectId(job['_id'])}, {"$set": {'status': 'failed', 'priority': 2}})
        else:
            result = ''
    else:
